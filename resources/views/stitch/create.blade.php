...
<body>
  <div class="container mt-5">
    <h2 class="mb-4">Stichting</h2>

    <!-- Form for submitting the data -->
    <form id="buyingForm" method="POST" action="{{ route('stitch.store') }}">
  ...
  <script>
    ...
    let newRow = `<tr id="row${rowCount}">
                    <td>${rowCount}</td>
                    <td><input type="text" class="form-control material" name="stitching_type[]" placeholder="Enter Type"></td>
                    <td><input type="number" class="form-control quantity" name="stitching_quantity[]" placeholder="Enter Quantity" min="1"></td>
                    <td><input type="number" class="form-control price" name="stitching_price[]" placeholder="Enter Price" min="0" step="0.01"></td>

...
<div class="container mt-5">
    <h2 class="mb-4">Edit Packing</h2>

    <form id="buyingForm" method="POST" action="{{ route('packing.update', $packingOrder->id) }}">
        @csrf
        @method('PUT')
...
<div class="mb-3">
    ...
    <input type="date" class="form-control" id="date" name="date" value="{{ $packingOrder->date }}" required>
        ...
        <table class="table table-bordered" id="myTable">
            ...
            <tbody>
                @foreach($packingMaterials as $index => $pack)
                    <tr id="row{{ $index }}">
                        <td>{{ $index + 1 }}</td>
                        <td><input type="text" class="form-control" name="packing_type[]" value="{{ $pack->packing_type }}" required></td>
                        <td><input type="number" class="form-control quantity" name="packing_quantity[]" value="{{ $pack->packing_quantity }}" min="1"></td>
                        <td><input type="number" class="form-control price" name="packing_price[]" value="{{ $pack->packing_price }}" min="0" step="0.01"></td>
